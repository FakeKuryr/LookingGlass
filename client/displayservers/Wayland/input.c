...
{
  const int32_t steps = waylandWheelFrame(&wheel);
  if (steps != 0)
  ...
  uint32_t serial, uint32_t axis, wl_fixed_t value)
{
  ...
  waylandWheelAxis(&wheel, delta);
  app_handleWheelMotion(delta / 15.0);
  ...
  uint32_t axis, int32_t discrete)
{
  ...
  waylandWheelDiscrete(&wheel, discrete);
}
#endif
  ...
  uint32_t axis, int32_t value120)
{
  ...
  waylandWheelValue120(&wheel, value120);
}
#endif
